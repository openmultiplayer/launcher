...
class Solver {
  ...
  solve() {
    ...
    return {
      ...
      loss: result.loss,
      filter: this.css(result.values ? result.values : []),
    };
  }
  ...
  solveWide() {
    ...
    for (let i = 0; best.loss > 25 && i < 3; i++) {
      ...
      if (result.loss < best.loss) {
        ...
        best.values = result.values ? [...result.values] : [];
      }
    }
    return best;
  }
...
function hexToRgb(hex: string) {
  ...
  hex = hex.replace(shorthandRegex, (_: any, r: any, g: any, b: any) => {
    return r + r + g + g + b + b;
  });
...
export const convertRgbToFilter = (color_: string) => {
  ...
  if (color_ === "#1A1A1E80") color_ = "#8C8C90";
